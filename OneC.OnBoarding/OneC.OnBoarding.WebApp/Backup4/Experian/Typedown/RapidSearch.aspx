<%@ Page language="c#" Inherits="com.qas.prowebintegration.RapidSearch" EnableEventValidation="false" CodeBehind="RapidSearch.aspx.cs" %>
...
%>
			...
			function checkLabel()
			{
			var labelText = document.getElementById("LabelPrompt").innerText;
			if (labelText == 'Enter search')
			{
				document.getElementById("LabelPrompt").innerText = "Enter street number and name. (e.g. '3330 Willard St')"
			}
			
			else if (labelText == 'Enter selection')
			{
				document.getElementById("LabelPrompt").innerText = "Choose address from the results below."
			}
			...
			function submitSearch()
			{
				...
				var sCurrent = document.getElementById("<%= searchText.ID %>").value;
				if (document.getElementById("RadioSingleline").checked && sCurrent != "")
				{
				 document.formAction.<%= searchText.ID %>.value = sCurrent + '*'
					...
					<td class="main">
						<div class="toolbar">
							<span class="first margin">
							    <button id="ButtonNew" style="width:40;height:25" type="button" runat="server" accessKey="N" onclick="preSubmit();" onserverclick="ButtonNew_ServerClick"><EM>N</EM>ew</button>
							    &nbsp;
							    <button id="ButtonBack" style="width:40;height:25" type="button" runat="server" accessKey="B" onclick="preSubmit();" onserverclick="ButtonBack_ServerClick"><EM>B</EM>ack</button>
							</span>
							<span class="margin">
...
<%--\								<asp:RadioButton id="RadioKeyfinder" accessKey="K" runat="server" Text="<em>K</em>ey Search" AutoPostBack="True" GroupName="GroupEngine" oncheckedchanged="RadioEngine_Changed"></asp:RadioButton>--%>
							...
							<span class="last margin">
							    <label for="country" id="lblCountry" ><em>C</em>ountry:</label>
            ...
            <input id="HiddenOrigin" type="hidden" runat="server" name="HiddenOrigin"/> 
            <        
		</form>
	</body>
</HTML>
