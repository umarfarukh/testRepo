...
$().ready(function () {
    ...
    OBPager.GetMaster(116, "takeupfortaxList");
    OBPager.GetMaster(116, "deppaidtaxList");
    ...
    jQXB.addOnTemplateItemBoundhnd(function (dataSourceName, templateName, occurrency, dataItem, jQrytemplateItem) {
        switch (templateName) {
            case "DependentTemplate": // filter only event databound from a specifica template
                ...
                var jQrytakeupfortax = jQrytemplateItem.find('.takeupfortax');
                var jQrydeppaidtax = jQrytemplateItem.find('.deppaidtax');
                ...
                var jQryCity_form = jQrytemplateItem.find('.cityKey');
                var jQryCountry_form = jQrytemplateItem.find('.countryKey');
                var jQryState_form = jQrytemplateItem.find('.stateKey');
                var jQryZipcode_form = jQrytemplateItem.find('.zipcodeKey');
                var jQryPhone_form = jQrytemplateItem.find('.phoneKey');
                ...
                if ($(jQrytakeupfortax).val() == null) {
                    $(jQrytakeupfortax).val(-1);
                }
                if ($(jQrydeppaidtax).val() == null) {
                    $(jQrydeppaidtax).val(-1);
                }
                ...
                if ($(jQryChk).is(':checked')) {
                    ...
                    $(jQryaddress_form).attr('readonly', true);
                    $(jQryCity_form).attr('readonly', true);
                    $(jQryCountry_form).attr('disabled', true);
                    $(jQryState_form).attr('disabled', true);
                    $(jQryZipcode_form).attr('readonly', true);
                    $(jQryaddresstype_form).attr('disabled', false);
                }
                else {
                    $(jQryaddresstype_form).val(-1);
                    $(jQryaddresstype_form).attr('disabled', true);
                    $(jQryaddress_form).attr('readonly', false);
                    $(jQryCity_form).attr('readonly', false);
                    $(jQryCountry_form).attr('disabled', false);
                    $(jQryState_form).attr('disabled', false);
                    $(jQryZipcode_form).attr('readonly', false);
                }
                ...
                if (jQryCheckPhone.is(':checked')) {
                    ...
                    $(jQryPhone_form).attr('disabled', true);
                    $(jQryPhone_form).attr('disabled', true);
    ...
    $(BaseData.DependentDetailsData.DependentRecordinfo.DependentsList).each(function () {
        ...
        if (cntrl > 1) {
            $('.statebirthobj').each(function () {
                ...
                var ChildDs = stateobj.attr('jqxb-dynamicChildDataSourceCountry');
                if (cntrl == birthcnt - 1 && birthCountry != null) {
                    var dynDS = ChildDs + G() + G();
                    stateobj.removeAttr('jqxb-listsource').attr('jqxb-listsource', dynDS);
...
function ChkforDisable(occurrency) {
    ...
    else if (occurrency > 1 && occurrency < 11) {
