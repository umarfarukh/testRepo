...
$().ready(function () {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        OBPager.SetTaskContentMemberValue('PersonalData.CurrentAddress2', null, TaskPrefillValues.PrefillValues.Set1.AddressDetails2, false);
    ...
    countryflag = $('#countryForm')[0].value;         //.find("option:selected").text();
    //    if (countryflag == 'United States') {
    //        $('#addressVerify').show();
    //    }
    //    else {
    //        $('#addressVerify').hide();
    //    }
    //    $('#countryForm option[value=" + 1 + "]').attr("selected", true);
    ssnCheck();
    if (OBPager.taskStatusFlag == -1) {
...
function SaveTaskData(saveMode) {
    ...
    if (saveMode == 1) {
        if (validate.ValidateSubmit() == true) {
            countryflag = $('#countryForm')[0].value;
            if (countryflag == 'UNITED STATES OF AMERICA' || countryflag == 'United States') {
               // if (vadddrflag == 1) {
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentAddress', null, $('#addressForm')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentAddress2', null, $('#address2Form')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentCity', null, $('#cityForm')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentState', null, $('#stateForm')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentPinCode', null, $('#zipCodeForm')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentCountry', null, $('#countryForm')[0].value, false);
                    OBPager.SetTaskContentMemberValue('PersonalData.CurrentCounty', null, $('#CountyForm')[0].value, false);
                    jQXB.doBind(OBPager.taskContentDSName);
...
//                }
//                else {
//                    MsgboxAlert(sessionId, 4, 0, null, 'Please verify your current address before you submit the form');
//                }

            }
            else if (countryflag != 'United States' || countryflag != 'UNITED STATES OF AMERICA') {
                if (OBPager.ValidateTaskData(saveMode) == true) {
                    try {
                        MsgboxConfirm(sessionId, 6, 217, 'FORM_ONSAVEORSUBMIT_CNFRM_NA', "<p style=line-height:20px;>Please verify the details on the personal data form before you submit as the data saved in this form will be displayed in all other forms.</p>", submitcallback);
                    }
...
function ResetTaskData() {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        OBPager.SetTaskContentMemberValue('PersonalData.CurrentPinCode', null, TaskPrefillValues.PrefillValues.Set1.PostalCode, false);
        OBPager.SetTaskContentMemberValue('PersonalData.CurrentCountry', null, TaskPrefillValues.PrefillValues.Set1.Country, false);
        OBPager.SetTaskContentMemberValue('PersonalData.CurrentCounty', null, TaskPrefillValues.PrefillValues.Set1.County, false);
        OBPager.SetTaskContentMemberValue('PersonalData.HomePh', null, TaskPrefillValues.PrefillValues.Set1.HomePhone, false);
        OBPager.SetTaskContentMemberValue('PersonalData.Mobile', null, TaskPrefillValues.PrefillValues.Set1.MobilePhone, false);
        OBPager.SetTaskContentMemberValue('PersonalData.EmailId', null, TaskPrefillValues.PrefillValues.Set1.PersonalEmail, false);
        OBPager.SetTaskContentMemberValue('PersonalData.EmergencyContactName', null, TaskPrefillValues.PrefillValues.Set1.ECName, false);
...
$(function () {
    ...
    $('#countryForm').change(function () {
        countryflag = $('#countryForm')[0].value;   //find("option:selected").text()
        if (countryflag == 'United States' || countryflag == 'UNITED STATES OF AMERICA') {
            $('#addressVerify').show();
        }
        else {
            $('#addressVerify').hide();
