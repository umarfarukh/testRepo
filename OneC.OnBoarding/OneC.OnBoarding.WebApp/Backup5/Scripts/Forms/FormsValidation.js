...
$(document).ready(function () {
    ...
    $('.AlphaNumericwithShift').keypress(function (event) {
        if (event.keyCode == 9 || event.keyCode == 48 || event.keyCode == 57) {

        }
        // prevents all special characters 
        if ((event.keyCode >= 33 && event.keyCode <= 47) || event.keyCode == 13 || (event.keyCode >= 58 && event.keyCode <= 64) || (event.keyCode >= 91 && event.keyCode <= 96) || (event.keyCode >= 123 && event.keyCode <= 126)) {
            event.preventDefault();
        }
        else {
        }
