...
function AjaxSucceeded(result) {
    ...
    if (response[0]["FileUploadStatus"] != -1 && response[0]["FileUploadId"] != 0) {
        ...
        //Functionality: Photo can be uploaded by admin till the ID card is Printed.
        //Upload functionality will be in open even after Approval of photo also. This is requested by Business.
        if (adminView == true) {
            ...
            if (status == UploadStatus.IDCardPrinted && qs["cntry"] == 175) {
                $('#submitButton').hide();
            }
            //IF IdCard Print and PhotoandNameApproved  disable Print Button
            if (status == UploadStatus.PhotoandNameApprove && qs["cntry"] == 175) {
            ...
            //Upload functionality will be in open even after Approval of photo also. This is requested by Business.
            else if (status == UploadStatus.PhotoandNameApprove) {
                $('#adminUploadDiv').show();
                $('#rejectDiv').hide();
                $('#Upload1').show();
                $('#Upload').hide();
            ...
            //Upload functionality will be freezed if ID card is printed.
            else if (status == UploadStatus.IDCardPrinted) {
                $('#adminUploadDiv').show();
                $('#rejectDiv').hide();
                $('#Upload1').show();
                $('#Upload').hide();
        ...
        if (saveMode == 1) {
            ...
            try {
                ...
                if (OBPager.SaveTaskData(saveMode) == true) {
                    ...
                    if (qs["cntry"] == 175 && qs["admin"] == undefined && (saveBstatus == UploadStatus.PendingWithAdminforApproval || saveBstatus == -1 || saveBstatus == UploadStatus.PhotoApproveNameReject || saveBstatus == UploadStatus.PhotoRejectNameApprove || saveBstatus == UploadStatus.PhotoandNameReject)) {
