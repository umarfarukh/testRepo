...
$().ready(function () {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.CurrentDate', null, (TaskPrefillValues.PrefillValues.Set1.CurrentDate), true);
    }
    if ((OBPager.taskStatusFlag == 1) || (OBPager.taskStatusFlag == 0)) {

        $('.currentDate').html(JSON.parse(OBPager.strFormDetails).BritishColumbiaTaxReturn.CurrentDate);
    }
    if ($("#Pg_1_text_BasicAmt").val() == "") {
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.BasicPersonalamt', null, 9869, true);
    }
    OBPager.ShowPage(1);
    ValidateOnlyInteger();
...
function ResetTaskData() {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        $("#Pg_1_text_SSN").val(TaskPrefillValues.PrefillValues.Set1.SSN);
        $("#Pg_2_text_SignatureDate").val(TaskPrefillValues.PrefillValues.Set1.SubmissionDate);
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.TotalClaimAmt', null, 9869, false);
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.BasicPersonalamt', null, 9869, true);
...
function CalculateClaimAmount(event) {
    ...
    if (Total.toString().indexOf(".") > 0) {
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.TotalClaimAmt', null, Total.toFixed(2), false);
        $("#Pg_1_text_TotalClaimAmt").val(Total.toFixed(2));
    }
    else {
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.TotalClaimAmt', null, parseInt(Total), false);
...
$('#Pg_2_check_Income').live("click", function (event) {
    ...
    if (a == true) {
        ...
        OBPager.SetTaskContentMemberValue('BritishColumbiaTaxReturn.TotalClaimAmt', null, '', true);
    }
    else {
        CalculateClaimAmount(event);
    }
