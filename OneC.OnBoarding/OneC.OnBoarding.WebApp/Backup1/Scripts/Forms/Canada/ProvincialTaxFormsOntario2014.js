$().ready(function () {
    ...
    if ($("#Pg_1_text_BasicAmt").val() == "") {
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.BasicPersonalamt', null, 9670, true);
    }
    if (OBPager.taskStatusFlag == -1) {
        $('.currentDate').html(TaskPrefillValues.PrefillValues.Set1.CurrentDate);
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.CurrentDate', null, (TaskPrefillValues.PrefillValues.Set1.CurrentDate), true);
    }
    if ((OBPager.taskStatusFlag == 1) || (OBPager.taskStatusFlag == 0)) {

        $('.currentDate').html(JSON.parse(OBPager.strFormDetails).ProvincialTaxOntario.CurrentDate);
    }
    OBPager.ShowPage(1);
    ValidateOnlyInteger();
...
function ResetTaskData() {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        $("#Pg_2_text_SignatureDate").val(TaskPrefillValues.PrefillValues.Set1.SubmissionDate);
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.BasicPersonalamt', null, 9670, true);
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.TotalClaimAmt', null, 9670, true);
...
function CalculateClaimAmount(event) {
    ...
    if (Total.toString().indexOf(".") > 0) {
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.TotalClaimAmt', null, Total.toFixed(2), false);
        $("#Pg_1_text_TotalClaimAmt").val(Total.toFixed(2));
    }
    else {
        OBPager.SetTaskContentMemberValue('ProvincialTaxOntario.TotalClaimAmt', null, parseInt(Total), false);
