...
$().ready(function () {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        OBPager.SetTaskContentMemberValue('PersonalTax.CurrentDate', null, (TaskPrefillValues.PrefillValues.Set1.CurrentDate), true);
    }
    if ((OBPager.taskStatusFlag == 1) || (OBPager.taskStatusFlag == 0)) {

        $('.currentDate').html(JSON.parse(OBPager.strFormDetails).PersonalTax.CurrentDate);
    }
    if ($("#Pg_1_text_BasicAmt").val() == "") {
        //$("#Pg_1_text_BasicAmt").val("10382");
        OBPager.SetTaskContentMemberValue('PersonalTax.BasicPersonalamt', null, 10382, true);
    }
    OBPager.ShowPage(1);
//    DisableCutCopyPaste();
    ValidateOnlyInteger();
...
function ResetTaskData() {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        $("#Pg_1_text_SSN").val(TaskPrefillValues.PrefillValues.Set1.SSN);
        $("#Pg_2_text_SignatureDate").val(TaskPrefillValues.PrefillValues.Set1.SubmissionDate);
        OBPager.SetTaskContentMemberValue('PersonalTax.BasicPersonalamt', null, 10382, false);
        OBPager.SetTaskContentMemberValue('PersonalTax.TotalClaimAmt', null, 10382, true);
...
function CalculateClaimAmount(event) {
    if ($("#Pg_2_check_NonResident").is(':checked')) { }
    else {
        var Total = 0;
        if ($("#Pg_1_text_BasicAmt").val() != "") {
        ...
        if ($("#Pg_1_text_ChildAmt").val() != "") {
            Total = Total + parseFloat($("#Pg_1_text_ChildAmt").val());
        }
        ...
        if (Total.toString().indexOf(".") > 0) {
            OBPager.SetTaskContentMemberValue('PersonalTax.TotalClaimAmt', null, Total.toFixed(2), false);
            $("#Pg_1_text_TotalClaimAmt").val(Total.toFixed(2));
        }
        else {
            OBPager.SetTaskContentMemberValue('PersonalTax.TotalClaimAmt', null, parseInt(Total), false);
