...
$().ready(function () {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.CurrentDate', null, (TaskPrefillValues.PrefillValues.Set1.CurrentDate), true);
    }
    if ((OBPager.taskStatusFlag == 1) || (OBPager.taskStatusFlag == 0)) {

        $('.currentDate').html(JSON.parse(OBPager.strFormDetails).ProvincialTaxALBERTA.CurrentDate);
    }
    if ($("#Pg_1_text_BasicAmt").val() == "") {
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.BasicPersonalamt', null, 17787, true);
    }
    OBPager.ShowPage(1);
    ValidateOnlyInteger();
...
function ResetTaskData() {
    ...
    if (OBPager.taskStatusFlag == -1) {
        ...
        $("#Pg_1_text_SSN").val(TaskPrefillValues.PrefillValues.Set1.SSN);
        $("#Pg_2_text_SignatureDate").val(TaskPrefillValues.PrefillValues.Set1.SubmissionDate);
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.BasicPersonalamt', null, 17787, false);
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.TotalClaimAmt', null, 17787, true);
...
function CalculateClaimAmount(event) {
    ...
    if (Total.toString().indexOf(".") > 0) {
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.TotalClaimAmt', null, Total.toFixed(2), false);
        $("#Pg_1_text_TotalClaimAmt").val(Total.toFixed(2));
    }
    else {
        OBPager.SetTaskContentMemberValue('ProvincialTaxALBERTA.TotalClaimAmt', null, parseInt(Total), false);
