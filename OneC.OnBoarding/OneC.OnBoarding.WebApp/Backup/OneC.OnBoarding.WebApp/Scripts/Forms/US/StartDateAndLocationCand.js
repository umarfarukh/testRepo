...
$(document).ready(function (e) {
    // OBPager.GetMaster(216, "NAPayRollLocationType");
    ...
    else if (TaskPrefillValues.PrefillValues.Set1.SMflag == 1) {
        $('#NormalNote').hide();
        $('#SMnote').show();
    }
    //    $('.clearfix').show();
    ...
    if (OBPager.taskStatusFlag == 1) {
        //  var wrkloctyp = TaskPrefillValues.PrefillValues.Set1.LocationType;
        locflag = TaskPrefillValues.PrefillValues.Set1.DOJLocationFlag;
        //        if (wrkloctyp == 1) {
        //            $('#WorkLocationType')[0].value = "Home";
        //        }
        //        else if (wrkloctyp == 2) {
        //            $('#WorkLocationType')[0].value = "Cognizant Office";
        //        }
        //        else if (wrkloctyp == 3) {
        //            $('#WorkLocationType')[0].value = "Client Office";
        //        }

        $('#addressContent').show();

        if (TaskPrefillValues.PrefillValues.Set1.ESHMStatus != 4) {
            $('#addressSubmit').hide();
            $('.button_wrapper').hide();
            $('.clearfix').hide();
            $('#datebutton').hide();
    ...
    //    // $('.wrapper').dialog({
    //    //    $("#startDateConfirmPopup").dialog({
    //    //        width: '564px',
    //    //        dialogClass: "na_popup",
    //    //        draggable: false,
    ...
    $('#dateConfirm').bind('click', function () {
        if (mdateflag == 1) {
            if ($('#txt_ModStatDt')[0].value == TaskPrefillValues.PrefillValues.Set1.DOJ) {
                $('#modfied_date_wrapper').hide();
                $('#datebutton').css({ 'margin-top': '-72px' });
                $('.start_date_wrapper').show();
            }
    ...
    $('#addressSubmit').bind('click', function () {
        ...
        //	if ($('#prsntResidanceAddress').hasClass('clicked')) {
        ...
        $("#" + popupId).dialog({
            ...
            buttons: [{ text: "Ok", 'class': "custom_button confirm_button", click: function () {
                if (popupId == 'confrimationMessagePopup') { popupIdStatus = 1; }
                else if (popupId == 'warningMessagePopup2') { popupIdHomeStatus = 1; }; $(this).dialog("close");
            }
            },
            //					{ text: "Cancel", click: function () { if (popupId == 'confrimationMessagePopup') { popupIdStatus = 0; }; $(this).dialog("close"); } }
                     ],
            open: function () {
...
function submitcall(popupIdHomeStatus) {
    if (popupIdHomeStatus == 1) {
        ...
        $("#" + popupId).dialog({
            ...
            draggable: false,
            show: { effect: 'fade', duration: 400 },
            buttons: [{ text: "Ok", 'class': "custom_button confirm_button", click: function () { if (popupId == 'confrimationMessagePopup') { popupIdStatus = 1; }; $(this).dialog("close"); } },
            //					{ text: "Cancel", click: function () { if (popupId == 'confrimationMessagePopup') { popupIdStatus = 0; }; $(this).dialog("close"); } }
                     ],
            open: function () {
                $('.warning_popup').prev('.ui-widget-overlay').css({ 'opacity': '0.45', 'z-index': '101' });
...
function forDateModify() {
    ...
    //  $('#datebutton').css({ ' margin-top': '-65px' });
...
function WorkAddressDisplay() {
    ...
    if (locflag == 3 || locflag == 4) {
        ...
        //$('#req_Address2')[0].value = TaskPrefillValues.PrefillValues.Set1.EAddress2;
    ...
    if (saveMode == 1) {
        if (validate.ValidateSubmit() == true) {
            try {
                ...
                //                if (loctype == "Home") {
                //                    OBPager.SetTaskContentMemberValue('NAPayRoll.LocationType', null, 1, false);
                //                }
                //                else if (loctype == "Cognizant Office") {
                //                    OBPager.SetTaskContentMemberValue('NAPayRoll.LocationType', null, 2, false);
                //                }
                //                else if (loctype == "Client Office") {
                //                    OBPager.SetTaskContentMemberValue('NAPayRoll.LocationType', null, 3, false);
                //                }
