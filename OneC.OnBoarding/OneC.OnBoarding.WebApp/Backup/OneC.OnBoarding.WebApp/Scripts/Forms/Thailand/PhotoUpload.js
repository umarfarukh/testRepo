...
function AjaxSucceeded(result) {
    ...
    if (response[0]["FileUploadStatus"] != -1 && response[0]["FileUploadId"] != 0) {
        ...
        if (adminView == true) {
            ...
            if (status == UploadStatus.IDCardPrinted && qs["cntry"] == 215) {
                $('#submitButton').hide();
            }
            //IF IdCard Print and PhotoandNameApproved  disable Print Button
            if (status == UploadStatus.PhotoandNameApprove && qs["cntry"] == 215) {
        ...
        if (saveMode == 1) {
            ...
            try {
                ...
                if (OBPager.SaveTaskData(saveMode) == true) {
                    ...
                    if (qs["cntry"] == 215 && qs["admin"] == undefined && (saveBstatus == UploadStatus.PendingWithAdminforApproval || saveBstatus == -1 || saveBstatus == UploadStatus.PhotoApproveNameReject || saveBstatus == UploadStatus.PhotoRejectNameApprove || saveBstatus == UploadStatus.PhotoandNameReject)) {
                        //  $('#lblNote').show();
