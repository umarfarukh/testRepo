...
function FetchPrefillvaluesdata(result) {
    ...
    $(xmldata).find('Table').each(function () {
        ...
        DOJ = $.trim(obj.find('DOJ').text());
        CandidateOfferStatus = obj.find('CandidateOfferStatus').text();
        DateIntimationStatus = obj.find('DateIntimationStatus').text();
        CampusJoiningLocation = obj.find('CampusJoiningLocation').text();
        CampusReportingTime = obj.find('CampusReportingTime').text();
        CampusJoiningVenue = $.trim(obj.find('CampusJoiningVenue').text());
        AttendanceStatusOnDOJ = obj.find('AttendanceStatusOnDOJ').text();
        IsAccessAllowed = obj.find('IsAccessAllowed').text();
    });
    $(xmldata).find('Table2').each(function () {
        var obj = $(this);
        RecruiterId = $.trim(obj.find('RecruiterId').text());
        RecruiterName = $.trim(obj.find('RecruiterName').text());
        RecruiterMobileNo = $.trim(obj.find('RecruiterMobileNumber').text());
        RecruiterVnetNo = $.trim(obj.find('RecruiterVnetNumber').text());
        ManagerId = $.trim(obj.find('ESAHiringManagerId').text());
        ManagerName = $.trim(obj.find('ESAHiringManagerName').text());
        ManagerMobileNo = $.trim(obj.find('ESAHiringManagerMobileNumber').text());
        ManagerVnetNo = $.trim(obj.find('ESAHiringManagerVnetNumber').text());
    });
}
