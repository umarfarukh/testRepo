...
(function($){
    ...
    var CarouselEvo = function(element, options){
        ...
        carousel.children('li').addClass('slideItem');
            ...
            if (p == 0){
                ...
                if (o.hAlign == 'center'){
					if (p > 0 && p <= Math.ceil((n-1)/2)){
						...
						//for buddy app
						hPos = o.carouselWidth - horizonPos(n-p) - setSlideSize(p).width + 10;
                    }                   
                }
                else {
					if (p == (n -1)){
        ...
        var setSlidePosition = function(p) {
            ...
            for (var i = 0; i < n; i++){
                ...
                if (o.hAlign == 'left'){
                    pos[i] = {width:sz.width, height:sz.height, top:topPos(i), left:horizonPos(i)};
                    if (o.vAlign == 'bottom'){
                        pos[i] = {width:sz.width, height:sz.height, bottom:topPos(i), left:horizonPos(i)} ;
                    }
                }
                else {
        ...
        var initSlides = function(){
            ...
            for (var i = 0; i < n; i++){
				...
				if(i==0){
                    slideItems.eq(slidePos(i)).addClass('ie_opacity');
				}

				if(i==1){
					//slideItems.eq(slidePos(i)).css({'-moz-transform':'skew(0deg,10deg)', '-ms-transform':'skew(0deg,10deg)'});
					slideItems.eq(slidePos(i)).addClass('blur_grey');
					slideItems.eq(slidePos(i)).find('.chat_content_me').addClass('chat_content_me_zoom_out');
					slideItems.eq(slidePos(i)).find('.chat_content_cindy').addClass('chat_content_cindy_zoom_out');
				}else if(i==(n-1)){
					//slideItems.eq(slidePos(i)).css({'-moz-transform':'skew(0deg,-10deg)','-ms-transform':'skew(0deg,-10deg)'});
					slideItems.eq(slidePos(i)).addClass('blur_grey');
					slideItems.eq(slidePos(i)).find('.chat_content_me').addClass('chat_content_me_zoom_out');
        ...
        var goTo = function(index, isStopAutoplay, isPause){
            ...
            o.before(self);
			$('.chat_content_me, .chat_content_cindy').hide();
            animateSlide();  
			setTimeout(function() {
				$('.chat_content_me, .chat_content_cindy').show();
			}, 500);
        ...
        var animateSlide = function(){
			...
			/* blur, grayscale and zoom out */
			
            //slideItems.eq(currentSlide).css({'-moz-transform':'skew(0deg,0deg)','-ms-transform':'skew(0deg,0deg)','-moz-transition-timing-function': 'ease-in-out'});
			slideItems.eq(currentSlide).removeClass('blur_grey').addClass('ie_opacity');
			slideItems.eq(currentSlide).find('.chat_content_me').removeClass('chat_content_me_zoom_out');
			slideItems.eq(currentSlide).find('.chat_content_cindy').removeClass('chat_content_cindy_zoom_out');
					
			var leftskew = currentSlide-1;
			var rightskew = currentSlide+1;
			if(currentSlide == 0){
				leftskew = n-1;
			}else if(currentSlide == n-1){
				rightskew = 0;
			}
			
			slideItems.eq(leftskew).addClass('blur_grey').removeClass('ie_opacity');
			slideItems.eq(rightskew).addClass('blur_grey').removeClass('ie_opacity');
			
			slideItems.eq(leftskew).find('.chat_content_me').addClass('chat_content_me_zoom_out');
			slideItems.eq(leftskew).find('.chat_content_cindy').addClass('chat_content_cindy_zoom_out');
			slideItems.eq(rightskew).find('.chat_content_me').addClass('chat_content_me_zoom_out');
			slideItems.eq(rightskew).find('.chat_content_cindy').addClass('chat_content_cindy_zoom_out');
			//slideItems.eq(rightskew).css({'-moz-transform':'skew(0deg,10deg)','-ms-transform':'skew(0deg,10deg)', '-moz-transition-timing-function': 'ease-in-out'});
			//slideItems.eq(leftskew).css({'-moz-transform':'skew(0deg,-10deg)','-ms-transform':'skew(0deg,-10deg)', '-moz-transition-timing-function': 'ease-in-out'});
            //animation    
            for (var i = 0; i < n; i++){
                animateImage(i);  
				//for buddy app
				if(i== currentSlide || i== leftskew || i== rightskew){
				}else{
					slideItems.eq(i).hide();
				}
            }            
        };        
